...
"""
collect_runs.py
Recursively collects all runs under --runs-dir, merged ml_dataset.csv + job_log.csv,
adds run_id and writes to out/ml_dataset.csv and out/job.csv.
"""
import argparse, sys
...
def run_id_from_path(p: Path, root: Path) -> str:
    # e.g. "util95_dueeng_K3_distlow/ATC_capBN177min/seed42/20251114_103144"
    rel = p.relative_to(root)
    return str(rel.parent).replace("\\", "/")

def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--runs-dir", default="runs", help="Root of the runs (recursive)")
    ap.add_argument("--min-rows", type=int, default=5, help="Minimum number of lines per file")
    args = ap.parse_args()
    ...
    if not keys:
        print("[WARN] No pairs (ml_dataset.csv + job_log.csv) found.", file=sys.stderr)
        sys.exit(1)
    ...
    if added == 0:
        print("[ERR] No suitable runs.", file=sys.stderr); sys.exit(2)
